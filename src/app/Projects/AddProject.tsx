  ...
  const handleScopeFileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Array.isArray(teamArr)) {
        alert(
          'Team must be a JSON array! Example: [{"name": "bob", "role": "designer", "availability": 20}]'
        );
        return;
      }
    } catch (e) {
      alert(
        'Team must be a valid JSON array! Example: [{"name": "bob", "role": "designer", "availability": 20}]'
      );
      return;
    }
    setLoading(true);
