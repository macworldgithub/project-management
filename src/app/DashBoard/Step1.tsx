...
const TimelineDashboard = () => {
  ...
  const chartData = React.useMemo(() => {
    ...
    const sortedTasks = [...timeline.tasks].sort((a, b) => new Date(a.endDate).getTime() - new Date(b.endDate).getTime());
    ...
    <div className="p-4 sm:p-6 md:p-8 bg-gray-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-6 text-black">
        ...
        <div className="bg-white p-4 rounded-lg shadow text-base">
          ...
          <div className="text-2xl font-bold mt-2">{timelines[selectedProjectId]?.tasks?.filter(t => t.status !== 'Completed').length ?? 0}</div>
        ...
        {loading ? (
          ...
          <>
            {!loading && (chartData === null) ? (
              <div className="text-center text-gray-500 py-10">No tasks available for this project.</div>
            ) :
              !loading && Array.isArray(chartData) && chartData.length === 0 ? (
                <div className="text-center text-gray-500 py-10">No timeline progress data available for this project.</div>
              ) : (
